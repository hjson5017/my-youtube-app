...
- 댓글 전체를 단어로 나눠, 자주 나온 단어 상위 20개를 가로 막대그래프로 보여줍니다.
- 댓글 전체로 워드클라우드 이미지를 만들어 보여줍니다 (한글 폰트 자동 다운로드).
...
import plotly.express as px   # 막대그래프 등 그래프를 쉽게 그리기 위한 라이브러리
from collections import Counter  # 단어 개수를 세기 위한 파이썬 기본 도구
import tempfile               # 다운로드한 폰트 파일을 임시로 저장하기 위한 파이썬 기본 도구
from wordcloud import WordCloud  # 워드클라우드 이미지를 만들기 위한 라이브러리
...
st.caption("유튜브 영상 링크를 넣으면, 좋아요가 많은 순으로 댓글을 최대 100개 가져오고 자주 나온 단어와 워드클라우드를 보여줍니다.")
...
# -----------------------------
# 12) 댓글에서 단어를 뽑아 자주 나온 단어 상위 20개 세기
#    - 한글/영문/숫자로 이루어진 덩어리를 하나의 '단어'로 봅니다.
#      (콤마, 마침표, 이모티콘, 특수문자 등은 단어 사이의 구분 기호로 취급되어 자동으로 제거됩니다)
#    - 한 글자짜리 단어(예: "이", "그", "a")는 의미 파악이 어려워 제외합니다.
# -----------------------------
def extract_words(text: str):
    """댓글 문장에서 한글/영문/숫자 덩어리를 단어 목록으로 뽑아 반환합니다."""
    if not text:
        return []
    # [가-힣]: 한글, [A-Za-z]: 영문, [0-9]: 숫자 -> 이 문자들이 이어진 덩어리를 하나의 단어로 인식
    return re.findall(r"[가-힣A-Za-z0-9]+", text)


all_words = []
for comment_text in df["댓글"]:
    all_words.extend(extract_words(comment_text))

# 한 글자짜리 단어는 제외
filtered_words = [word for word in all_words if len(word) >= 2]

# Counter로 단어별 등장 횟수를 세어둡니다 (Top 20 그래프, 워드클라우드에서 함께 사용)
word_counter = Counter(filtered_words) if filtered_words else None

st.subheader("🔤 자주 나온 단어 Top 20")

if not filtered_words:
    # 댓글에서 두 글자 이상인 단어를 하나도 못 찾은 경우 (이모티콘/특수문자 위주의 댓글 등)
    st.info("분석할 수 있는 단어(두 글자 이상)를 댓글에서 찾지 못했습니다.")
else:
    # 가장 많이 나온 20개만 추림
    top_20_words = word_counter.most_common(20)

    word_df = pd.DataFrame(top_20_words, columns=["단어", "빈도수"])

    # 빈도수가 낮은 단어부터 정렬해두면, 그래프에서 가장 많이 나온 단어가 자연스럽게 맨 위로 갑니다.
    word_df = word_df.sort_values("빈도수", ascending=True)
    ...
    fig = px.bar(
        ...
        text="빈도수",      # 막대 끝에 숫자를 표시
    )
    fig.update_traces(textposition="outside")
    fig.update_layout(
        xaxis_title="등장 횟수",
        yaxis_title="",
        height=600,  # 20개 항목이 잘 보이도록 그래프 높이를 넉넉하게 설정
    )
...
# -----------------------------
# 13) 워드클라우드 그리기
#    - 한글이 깨지지 않도록, 한글을 지원하는 나눔고딕 폰트를 인터넷에서 내려받아 사용합니다.
#    - matplotlib은 사용하지 않고, wordcloud 라이브러리가 만들어주는 이미지를 st.image로 바로 띄웁니다.
# -----------------------------
NANUM_FONT_URL = (
...
@st.cache_resource(ttl=86400)  # 폰트는 하루 동안 캐시해서, 매번 새로 내려받지 않도록 함
def download_korean_font():
    """
    나눔고딕 폰트 파일을 인터넷에서 내려받아 임시 파일로 저장하고, 그 경로를 반환합니다.
    다운로드에 실패하면 None을 반환합니다.
    """
    try:
        response = requests.get(NANUM_FONT_URL, timeout=15)
        response.raise_for_status()  # 응답 코드가 200이 아니면 예외를 발생시킴
    except requests.exceptions.RequestException:
        return None

    # 임시 파일(.ttf)로 폰트 내용을 저장해두고, 그 파일 경로를 wordcloud에 넘겨줍니다.
...
if font_path is None:
    # 폰트 다운로드에 실패한 경우 (인터넷 연결 문제 등)
    st.warning(
        "😥 한글 폰트를 내려받지 못해 워드클라우드를 만들 수 없습니다.\n\n"
        "잠시 후 페이지를 새로고침해서 다시 시도해 주세요."
    )
elif not filtered_words:
    # 워드클라우드로 그릴 단어(두 글자 이상)가 없는 경우
    st.info("워드클라우드를 그릴 단어(두 글자 이상)를 댓글에서 찾지 못했습니다.")
else:
    # 댓글 전체 단어 빈도(word_counter)를 바탕으로 워드클라우드 생성 (배경은 흰색)
    ...
    # WordCloud가 만들어주는 이미지를 그대로 화면에 띄움 (matplotlib 사용 안 함)
    st.image(wordcloud_image.to_image(), use_container_width=True)
